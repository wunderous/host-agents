...
func appendGenericHostDefinitions(defs []ToolDefinition) []ToolDefinition {
	...
	defs = append(defs, ToolDefinition{
		...
		Name: "reconcile_postgresql_service", Title: "Reconcile PostgreSQL service", Description: "Reconcile a caller-defined PostgreSQL service and databases on an explicit Kubernetes target. Credentials remain operator-owned and are never returned.", InputSchema: map[string]any{"type": "object", "required": []string{"vmName", "databases", "consumerSecretName", "consumerSecretLabel", "serviceOwner", "servicePartOf", "consumerDatabaseKeys"}, "properties": map[string]any{"vmName": map[string]any{"type": "string"}, "clusterName": map[string]any{"type": "string"}, "namespace": map[string]any{"type": "string"}, "instances": map[string]any{"type": "integer"}, "storageClass": map[string]any{"type": "string"}, "storageSize": map[string]any{"type": "string"}, "retentionPolicy": map[string]any{"type": "string", "enum": []string{"delete", "retain"}}, "databases": map[string]any{"type": "array", "minItems": 1, "items": map[string]any{"type": "string"}}, "consumerSecretName": map[string]any{"type": "string"}, "consumerSecretLabel": map[string]any{"type": "string"}, "serviceOwner": map[string]any{"type": "string"}, "servicePartOf": map[string]any{"type": "string"}, "consumerDatabaseKeys": map[string]any{"type": "object", "additionalProperties": map[string]any{"type": "string"}}, "relayDeviceName": map[string]any{"type": "string"}, "restartConsumers": map[string]any{"type": "boolean"}, "localRelay": postgresqlServiceRelaySchema()}}, OutputSchema: map[string]any{"type": "object"},
	}, ToolDefinition{
		Name: "install_helm_chart", Title: "Install Helm chart", Description: "Deploy a Helm chart through the K3s Helm Controller on an explicit VM-backed Kubernetes target. The caller's vmName is retained for durable evidence; execution is admitted only through its canonical instance URI.", InputSchema: map[string]any{"type": "object", "required": []string{"vmName", "uri"}, "properties": map[string]any{"vmName": map[string]any{"type": "string"}, "uri": map[string]any{"type": "string", "pattern": `^(vm|container):[a-z][a-z0-9-]{0,31}:.+$`}, "name": map[string]any{"type": "string"}, "chartName": map[string]any{"type": "string"}, "releaseName": map[string]any{"type": "string"}, "chart": map[string]any{"type": "string"}, "chartSource": map[string]any{"type": "string"}, "namespace": map[string]any{"type": "string"}, "repo": map[string]any{"type": "string"}, "values": map[string]any{"type": "object", "additionalProperties": true}}}, OutputSchema: map[string]any{"type": "object"}, Meta: map[string]any{"requires": []any{map[string]any{"argument": "uri", "resourceType": "vm", "required": true}, map[string]any{"argument": "uri", "resourceType": "container", "required": true}}},
	}, ToolDefinition{
		Name: "get_postgresql_service_status", Title: "Get PostgreSQL service status", Description: "Read SQL-gated readiness for a caller-defined PostgreSQL service without returning credentials.", InputSchema: map[string]any{"type": "object", "required": []string{"vmName", "databases", "consumerSecretName", "consumerSecretLabel", "serviceOwner", "servicePartOf", "consumerDatabaseKeys"}, "properties": map[string]any{"vmName": map[string]any{"type": "string"}, "clusterName": map[string]any{"type": "string"}, "namespace": map[string]any{"type": "string"}, "instances": map[string]any{"type": "integer"}, "databases": map[string]any{"type": "array", "minItems": 1, "items": map[string]any{"type": "string"}}, "consumerSecretName": map[string]any{"type": "string"}, "consumerSecretLabel": map[string]any{"type": "string"}, "serviceOwner": map[string]any{"type": "string"}, "servicePartOf": map[string]any{"type": "string"}, "consumerDatabaseKeys": map[string]any{"type": "object", "additionalProperties": map[string]any{"type": "string"}}, "relayDeviceName": map[string]any{"type": "string"}, "localRelay": postgresqlServiceRelaySchema()}}, OutputSchema: map[string]any{"type": "object"},
	}, ToolDefinition{
		Name: "remove_postgresql_service", Title: "Remove PostgreSQL service", Description: "Remove a caller-defined PostgreSQL service and owned data after explicit confirmation.", InputSchema: map[string]any{"type": "object", "required": []string{"vmName", "databases", "consumerSecretName", "consumerSecretLabel", "serviceOwner", "servicePartOf", "consumerDatabaseKeys", "confirm"}, "properties": map[string]any{"vmName": map[string]any{"type": "string"}, "clusterName": map[string]any{"type": "string"}, "namespace": map[string]any{"type": "string"}, "databases": map[string]any{"type": "array", "minItems": 1, "items": map[string]any{"type": "string"}}, "consumerSecretName": map[string]any{"type": "string"}, "consumerSecretLabel": map[string]any{"type": "string"}, "serviceOwner": map[string]any{"type": "string"}, "servicePartOf": map[string]any{"type": "string"}, "consumerDatabaseKeys": map[string]any{"type": "object", "additionalProperties": map[string]any{"type": "string"}}, "retentionPolicy": map[string]any{"type": "string", "enum": []string{"delete", "retain"}}, "confirm": map[string]any{"type": "boolean"}}}, OutputSchema: map[string]any{"type": "object"},
	}, ToolDefinition{
		...
		Name:         "inspect_guest_storage",
		Title:        "Inspect guest cluster storage",
		Description:  "Inspect k3s/containerd image, snapshot, and PVC usage inside the bound cluster guest without changing state. Optional registry tag inventory.",
		InputSchema:  map[string]any{"type": "object", "required": []string{"uri"}, "properties": map[string]any{"uri": map[string]any{"type": "string", "minLength": 1, resourceTypeKeyword: "cluster"}, "includeRegistry": map[string]any{"type": "boolean"}, "extraKeepTags": map[string]any{"type": "array", "items": map[string]any{"type": "string"}}, "registryNamespace": map[string]any{"type": "string"}, "registryName": map[string]any{"type": "string"}}},
		OutputSchema: map[string]any{"type": "object"},
		Meta:         map[string]any{"requires": []any{map[string]any{"argument": "uri", "resourceType": "cluster", "required": true}}},
	}, ToolDefinition{
		Name:         "prune_unused_cluster_images",
		Title:        "Prune unused cluster images",
		Description:  "Age-gated removal of unused k3s/containerd images not referenced by current pods. Dry-run first. Never deletes PVCs or Incus instances.",
		InputSchema:  map[string]any{"type": "object", "required": []string{"uri"}, "properties": map[string]any{"uri": map[string]any{"type": "string", "minLength": 1, resourceTypeKeyword: "cluster"}, "dryRun": map[string]any{"type": "boolean"}, "minAgeSeconds": map[string]any{"type": "integer", "minimum": 3600}, "extraKeepTags": map[string]any{"type": "array", "items": map[string]any{"type": "string"}}}},
		OutputSchema: map[string]any{"type": "object"},
		Meta:         map[string]any{"needsApproval": true, "resourceCost": map[string]any{"class": "heavy", "cpuCores": 2, "memoryBytes": 2147483648, "tasks": 8}, "requires": []any{map[string]any{"argument": "uri", "resourceType": "cluster", "required": true}}},
	}, ToolDefinition{
		Name:         "garbage_collect_cluster_registry",
		Title:        "Garbage-collect cluster registry",
		Description:  "Delete untagged/unkept in-cluster registry manifests, scale the registry to zero, run registry garbage-collect --delete-untagged, restore, and wait Ready. Default off via includeRegistry. The GC Job omits a configMap volume when the live deploy has none.",
		...
		Name:         "trim_guest_storage",
		Title:        "TRIM guest filesystem",
		Description:  "Run guest fstrim after reclaim. Unprivileged Incus guests that get FITRIM EPERM fall back to host sudo -n fstrim -v / and report scope guest|host.",
		InputSchema:  map[string]any{"type": "object", "required": []string{"uri"}, "properties": map[string]any{"uri": map[string]any{"type": "string", "minLength": 1, resourceTypeKeyword: "cluster"}}},
		OutputSchema: map[string]any{"type": "object"},
		Meta:         map[string]any{"needsApproval": true, "requires": []any{map[string]any{"argument": "uri", "resourceType": "cluster", "required": true}}},
	}, ToolDefinition{
		Name:        "cleanup_container_storage",
		Title:       "Clean up container runtime storage",
		Description: "Age-gated cleanup of unused host Podman images and supported build cache; containers, volumes, networks, and running image references are preserved. Shared multi-tag images are skipped until a tag is retired. Does not remove buildah working containers.",
		InputSchema: map[string]any{"type": "object", "properties": map[string]any{
			"runtime":       map[string]any{"type": "string", "enum": []string{"auto", "podman"}},
			"maxBytes":      map[string]any{"type": "integer", "minimum": 0},
			"minAgeSeconds": map[string]any{"type": "integer", "minimum": 3600},
		...
		Name:         "compact_wsl_disk",
		Title:        "Compact WSL VHDX",
		Description:  "Inspect and compact a Stopped WSL2 distro VHDX with Optimize-VHD Full (needs an elevated Hyper-V admin token). Fails closed if the distro is live, Running, or the VHDX is still locked by a sibling WSL VM; compact then needs shutdown_wsl (kills this agent). Does not enable sparse VHDX. Not a recipe node.",
		InputSchema:  map[string]any{"type": "object", "required": []string{"distro"}, "properties": map[string]any{"distro": map[string]any{"type": "string", "minLength": 1}, "dryRun": map[string]any{"type": "boolean"}}},
		OutputSchema: map[string]any{"type": "object", "required": []string{"distro"}},
		Meta:         map[string]any{"needsApproval": true, "resourceCost": map[string]any{"class": "heavy", "cpuCores": 2, "memoryBytes": 2147483648, "tasks": 8}},
	}, ToolDefinition{
		Name:        "apply_manifest",
