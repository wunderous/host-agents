...
func StandaloneToolDefinitions() []ToolDefinition {
	defs := []ToolDefinition{
		...
		{Name: "ensure_host_file", Description: "Atomically reconcile a caller-declared managed file; user scope is home-owned and system scope is limited to systemd service units.", InputSchema: objectSchema(map[string]any{"path": map[string]any{"type": "string", "minLength": 1}, "content": map[string]any{"type": "string", "writeOnly": true}, "mode": map[string]any{"type": "integer", "minimum": 384, "maximum": 493}, "scope": map[string]any{"type": "string", "enum": []string{"user", "system"}}}, []string{"path", "content"}), OutputSchema: map[string]any{"type": "object", "required": []string{"path", "changed", "contentSha256", "mode"}}},
		...
		{Name: "ensure_public_mcp_tunnel", Description: "Install a pinned cloudflared connector for one provider-issued public MCP binding, store its token in an Opute-owned secret file, and prove authenticated tools/list at the stable HTTPS endpoint.", InputSchema: objectSchema(map[string]any{
			...
			"originHostId":   map[string]any{"type": "string", "minLength": 1, "description": "Exact enrolled Host Agent identity for a non-loopback localTarget."},
		...
		}, []string{"bindingId", "endpoint", "localTarget", "tunnelToken"}), OutputSchema: map[string]any{"type": "object", "required": []string{"contractVersion", "ready", "bindingId", "endpoint", "localTarget", "originHostId", "scope", "serviceName", "serviceFile", "tokenFile", "artifact", "origin", "publicAuth"}}, Meta: map[string]any{"effect": "credential_bearing", "needsApproval": true, "resourceCost": map[string]any{"class": "heavy", "cpuCores": 2, "memoryBytes": 2147483648, "tasks": 8}}},
		{Name: "ensure_public_mcp_quick_tunnel", Description: "Install a pinned cloudflared Quick Tunnel for this Host Agent's loopback MCP endpoint and prove authenticated tools/list. The returned trycloudflare.com URL is ephemeral and never a stable production binding.", InputSchema: objectSchema(map[string]any{
			"bindingId":      map[string]any{"type": "string", "pattern": `^[A-Za-z0-9][A-Za-z0-9._:-]{0,254}$`},
			"localTarget":    map[string]any{"type": "string", "format": "uri", "description": "This Host Agent MCP origin, ending exactly in /mcp and resolving to loopback."},
			"artifactUri":    map[string]any{"type": "string", "format": "uri"},
			"artifactSha256": map[string]any{"type": "string", "pattern": `^(sha256:)?[0-9a-fA-F]{64}$`},
			"scope":          map[string]any{"type": "string", "enum": []string{"user", "system"}},
		}, []string{"bindingId", "localTarget"}), OutputSchema: map[string]any{"type": "object", "required": []string{"contractVersion", "ready", "mode", "stable", "bindingId", "endpoint", "localTarget", "scope", "serviceName", "serviceFile", "logFile", "stateFile", "artifact", "origin", "publicAuth"}}, Meta: map[string]any{"effect": "mutation", "needsApproval": true, "resourceCost": map[string]any{"class": "heavy", "cpuCores": 2, "memoryBytes": 2147483648, "tasks": 8}}},
		{Name: "remove_public_mcp_quick_tunnel", Description: "Stop and remove one Opute-owned Cloudflare Quick Tunnel and its verified artifact after explicit confirmation; unrelated paths are refused.", InputSchema: objectSchema(map[string]any{
			"bindingId":      map[string]any{"type": "string", "pattern": `^[A-Za-z0-9][A-Za-z0-9._:-]{0,254}$`},
			"artifactUri":    map[string]any{"type": "string", "format": "uri"},
			"artifactSha256": map[string]any{"type": "string", "pattern": `^(sha256:)?[0-9a-fA-F]{64}$`},
			"scope":          map[string]any{"type": "string", "enum": []string{"user", "system"}},
			"confirm":        map[string]any{"type": "boolean"},
		}, []string{"bindingId", "confirm"}), OutputSchema: map[string]any{"type": "object", "required": []string{"contractVersion", "ready", "status", "bindingId", "scope", "serviceName", "attempts"}}, Meta: map[string]any{"effect": "destructive", "needsApproval": true, "resourceCost": map[string]any{"class": "heavy", "cpuCores": 2, "memoryBytes": 2147483648, "tasks": 8}}},
		...
		{Name: "reconcile_postgresql_service", Description: "Reconcile a caller-defined PostgreSQL service and databases on an explicit Kubernetes target.", InputSchema: objectSchema(map[string]any{"vmName": map[string]any{"type": "string"}, "clusterName": map[string]any{"type": "string"}, "namespace": map[string]any{"type": "string"}, "instances": map[string]any{"type": "integer"}, "storageClass": map[string]any{"type": "string"}, "storageSize": map[string]any{"type": "string"}, "retentionPolicy": map[string]any{"type": "string"}, "databases": map[string]any{"type": "array", "minItems": 1, "items": map[string]any{"type": "string"}}, "consumerSecretName": map[string]any{"type": "string", "minLength": 1}, "consumerSecretLabel": map[string]any{"type": "string", "minLength": 1}, "serviceOwner": map[string]any{"type": "string", "minLength": 1}, "servicePartOf": map[string]any{"type": "string", "minLength": 1}, "consumerDatabaseKeys": map[string]any{"type": "object", "additionalProperties": map[string]any{"type": "string"}}, "relayDeviceName": map[string]any{"type": "string"}, "restartConsumers": map[string]any{"type": "boolean"}, "localRelay": postgresqlServiceRelaySchema()}, []string{"vmName", "databases", "consumerSecretName", "consumerSecretLabel", "serviceOwner", "servicePartOf", "consumerDatabaseKeys"})},
		{Name: "get_postgresql_service_status", Description: "Read SQL-gated readiness for a caller-defined PostgreSQL service.", InputSchema: objectSchema(map[string]any{"vmName": map[string]any{"type": "string"}, "clusterName": map[string]any{"type": "string"}, "namespace": map[string]any{"type": "string"}, "instances": map[string]any{"type": "integer"}, "databases": map[string]any{"type": "array", "minItems": 1, "items": map[string]any{"type": "string"}}, "consumerSecretName": map[string]any{"type": "string", "minLength": 1}, "consumerSecretLabel": map[string]any{"type": "string", "minLength": 1}, "serviceOwner": map[string]any{"type": "string", "minLength": 1}, "servicePartOf": map[string]any{"type": "string", "minLength": 1}, "consumerDatabaseKeys": map[string]any{"type": "object", "additionalProperties": map[string]any{"type": "string"}}, "relayDeviceName": map[string]any{"type": "string"}, "localRelay": postgresqlServiceRelaySchema()}, []string{"vmName", "databases", "consumerSecretName", "consumerSecretLabel", "serviceOwner", "servicePartOf", "consumerDatabaseKeys"})},
		{Name: "remove_postgresql_service", Description: "Remove a caller-defined PostgreSQL service after explicit confirmation.", InputSchema: objectSchema(map[string]any{"vmName": map[string]any{"type": "string"}, "clusterName": map[string]any{"type": "string"}, "namespace": map[string]any{"type": "string"}, "databases": map[string]any{"type": "array", "minItems": 1, "items": map[string]any{"type": "string"}}, "consumerSecretName": map[string]any{"type": "string", "minLength": 1}, "consumerSecretLabel": map[string]any{"type": "string", "minLength": 1}, "serviceOwner": map[string]any{"type": "string", "minLength": 1}, "servicePartOf": map[string]any{"type": "string", "minLength": 1}, "consumerDatabaseKeys": map[string]any{"type": "object", "additionalProperties": map[string]any{"type": "string"}}, "retentionPolicy": map[string]any{"type": "string"}, "confirm": map[string]any{"type": "boolean"}}, []string{"vmName", "databases", "consumerSecretName", "consumerSecretLabel", "serviceOwner", "servicePartOf", "consumerDatabaseKeys", "confirm"})},
		...
		{Name: "cancel_operation", Description: "Cancel a local standalone operation.", InputSchema: objectSchema(map[string]any{"operationId": map[string]any{"type": "string"}}, []string{"operationId"}), Meta: map[string]any{"resourceCost": map[string]any{"class": "control"}}},
		...
		{Name: "validate_host_local_recipe", Description: "Resolve and validate a host-recipe.v1 document that declares coordinator=host-agent/mode=local, against this Host Agent's catalog and identity, without changing host state.", InputSchema: objectSchema(map[string]any{"source": map[string]any{"type": "string", "minLength": 1}, "revision": map[string]any{"type": "string"}, "sha256": map[string]any{"type": "string"}, "inputs": map[string]any{"type": "object"}}, []string{"source"}), OutputSchema: map[string]any{"type": "object", "required": []string{"valid", "recipeId", "recipeVersion", "recipeHash", "plan"}}, Meta: map[string]any{"resourceCost": map[string]any{"class": "control"}}},
		{Name: "run_host_local_recipe", Description: "Execute a host-recipe.v1 document that declares coordinator=host-agent/mode=local through the durable host-plan runner, so a first cluster node can be established with no Platform in the loop. Read the run with get_host_plan_run. A distributed recipe is refused.", InputSchema: objectSchema(map[string]any{"source": map[string]any{"type": "string", "minLength": 1}, "revision": map[string]any{"type": "string"}, "sha256": map[string]any{"type": "string"}, "inputs": map[string]any{"type": "object"}, "resume": map[string]any{"type": "boolean"}}, []string{"source"}), OutputSchema: map[string]any{"type": "object", "required": []string{"runId", "status", "catalogRevision"}}, Meta: map[string]any{"resourceCost": map[string]any{"class": "heavy", "cpuCores": 2, "memoryBytes": 2147483648, "tasks": 8}}},
		...
		{Name: "validate_tunnel_recipe", Description: "Resolve and validate an external tunnel-recipe.v1 source and its embedded host-plan.v1 without changing host state.", InputSchema: objectSchema(map[string]any{"source": map[string]any{"type": "string", "minLength": 1}, "revision": map[string]any{"type": "string"}, "sha256": map[string]any{"type": "string"}, "inputs": map[string]any{"type": "object"}}, []string{"source"}), Meta: map[string]any{"resourceCost": map[string]any{"class": "control"}}},
		...
		{Name: "opute.provider.teardown", Description: "Ask a connected provider for a generic teardown plan, execute it durably after explicit confirmation, and retire the provider generation only after success. The host also reclaims the provider's own service unit, resolving which unit that is from the provider's installed location, and refuses rather than retiring a generation whose process it could not identify; pass inputs.serviceName to name it explicitly, or inputs.hostService=\"none\" for a provider supervised elsewhere. With force=true, a generation whose provider process is gone is reclaimed host-side after its own prepare callback has failed; the provider's external resources are not finalized and remain the operator's to remove.", InputSchema: objectSchema(map[string]any{"provider": map[string]any{"type": "string", "minLength": 1}, "generation": map[string]any{"type": "string", "minLength": 1}, "inputs": map[string]any{"type": "object"}, "confirm": map[string]any{"type": "boolean"}, "force": map[string]any{"type": "boolean"}, "resume": map[string]any{"type": "boolean"}}, []string{"provider", "confirm"}), OutputSchema: map[string]any{"type": "object", "required": []string{"runId", "status", "catalogRevision"}}, Meta: map[string]any{"resourceCost": map[string]any{"class": "heavy", "cpuCores": 2, "memoryBytes": 2147483648, "tasks": 8}}},
		...
		{Name: "install_cloudflared_connector", Description: "Deploy a token-backed Cloudflare connector inside Kubernetes, with optional generic local-port-to-service mappings.", InputSchema: objectSchema(map[string]any{"vmName": map[string]any{"type": "string"}, "namespace": map[string]any{"type": "string"}, "name": map[string]any{"type": "string"}, "token": map[string]any{"type": "string"}, "image": map[string]any{"type": "string"}, "replicas": map[string]any{"type": "integer"}, "forwarderImage": map[string]any{"type": "string"}, "localTargets": map[string]any{"type": "array", "items": map[string]any{"type": "object", "required": []string{"localPort", "target"}, "properties": map[string]any{"localPort": map[string]any{"type": "integer", "minimum": 1, "maximum": 65535}, "target": map[string]any{"type": "string", "minLength": 3}}}}}, []string{"vmName", "token"})},
		...
		{Name: "uninstall_incus_stack", Description: "Purge the Incus virtualization stack from this host. Refuses while any instance remains; opt in to discarding /var/lib/incus.", InputSchema: objectSchema(map[string]any{"confirm": map[string]any{"type": "boolean"}, "removeState": map[string]any{"type": "boolean"}, "keepRepository": map[string]any{"type": "boolean"}}, []string{"confirm"})},
