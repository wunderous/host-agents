...
func StandaloneToolDefinitions() []ToolDefinition {
	defs := []ToolDefinition{
		...
		{Name: "ensure_platform_postgres", Description: "Install or reconcile CloudNativePG and a SQL-gated platform PostgreSQL Cluster in the host-agent-managed K3s VM. Credentials remain CNPG-owned and are never returned.", InputSchema: objectSchema(map[string]any{"vmName": map[string]any{"type": "string"}, "clusterName": map[string]any{"type": "string"}, "namespace": map[string]any{"type": "string"}, "instances": map[string]any{"type": "integer"}, "storageClass": map[string]any{"type": "string"}, "storageSize": map[string]any{"type": "string"}, "retentionPolicy": map[string]any{"type": "string", "enum": []any{"delete", "retain"}}, "localRelay": map[string]any{"type": "object"}}, []string{"vmName"})},
		{Name: "get_platform_postgres_status", Description: "Read SQL-gated CloudNativePG platform PostgreSQL readiness without returning credentials.", InputSchema: objectSchema(map[string]any{"vmName": map[string]any{"type": "string"}, "clusterName": map[string]any{"type": "string"}, "namespace": map[string]any{"type": "string"}, "instances": map[string]any{"type": "integer"}}, []string{"vmName"})},
		{Name: "remove_platform_postgres", Description: "Destructively remove the platform PostgreSQL CNPG Cluster and owned data while preserving the operator. Requires confirm=true.", InputSchema: objectSchema(map[string]any{"vmName": map[string]any{"type": "string"}, "clusterName": map[string]any{"type": "string"}, "namespace": map[string]any{"type": "string"}, "retentionPolicy": map[string]any{"type": "string", "enum": []any{"delete", "retain"}}, "localRelay": map[string]any{"type": "object"}, "confirm": map[string]any{"type": "boolean"}}, []string{"vmName", "confirm"})},
		...
		{Name: "probe_local_llm", Description: "Probe the local Ollama endpoint. Optionally warm-load modelRef or modelPreset (granite|nemotron) with numGpu/numCtx and return loadError/remediationHints plus GPU sizeVramBytes.", InputSchema: objectSchema(map[string]any{"includeChat": map[string]any{"type": "boolean"}, "modelRef": map[string]any{"type": "string"}, "modelPreset": map[string]any{"type": "string", "enum": []any{"granite", "nemotron"}}, "numGpu": map[string]any{"type": "integer"}, "numCtx": map[string]any{"type": "integer"}}, nil)},
		{Name: "install_local_llm_model", Description: "Install an Opute-managed Ollama runtime and model.", InputSchema: objectSchema(map[string]any{"modelFamily": map[string]any{"type": "string"}, "modelVariant": map[string]any{"type": "string"}, "modelRef": map[string]any{"type": "string"}, "modelPreset": map[string]any{"type": "string", "enum": []any{"granite", "nemotron"}}, "startRuntime": map[string]any{"type": "boolean"}, "createAs": map[string]any{"type": "string"}, "numGpu": map[string]any{"type": "integer"}, "numCtx": map[string]any{"type": "integer"}, "template": map[string]any{"type": "string"}}, nil)},
		{Name: "configure_local_llm_model", Description: "Create/replace a local Ollama tag FROM an already-pulled model with Modelfile parameters (numGpu/numCtx). Does not re-download.", InputSchema: objectSchema(map[string]any{"modelRef": map[string]any{"type": "string"}, "modelPreset": map[string]any{"type": "string", "enum": []any{"granite", "nemotron"}}, "fromRef": map[string]any{"type": "string"}, "numGpu": map[string]any{"type": "integer"}, "numCtx": map[string]any{"type": "integer"}, "template": map[string]any{"type": "string"}}, nil)},
		...
		{Name: "configure_local_llm_runtime", Description: "Persist and apply Opute-managed Ollama GPU/runtime limits (gpuOverheadMiB, maxLoadedModels, numParallel, flashAttention) by re-rendering the managed systemd unit and restarting the runtime. Omitted fields keep current values.", InputSchema: objectSchema(map[string]any{"gpuOverheadMiB": map[string]any{"type": "integer"}, "maxLoadedModels": map[string]any{"type": "integer"}, "numParallel": map[string]any{"type": "integer"}, "flashAttention": map[string]any{"type": "boolean"}}, nil)},
		{Name: "stop_local_llm_runtime", Description: "Stop the local Ollama runtime.", InputSchema: objectSchema(nil, nil)},
		{Name: "remove_local_llm_model", Description: "Remove a local Ollama model. Set purge=true to wipe all managed models.", InputSchema: objectSchema(map[string]any{"modelRef": map[string]any{"type": "string"}, "purge": map[string]any{"type": "boolean"}}, []string{"modelRef"})},
		...
		{Name: "discover_cluster_ingress", Description: "Resolve Traefik/VM bridge endpoints for CPC web and MCP without kubectl port-forward.", InputSchema: objectSchema(map[string]any{
