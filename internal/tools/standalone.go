...
func StandaloneToolDefinitions() []ToolDefinition {
	defs := []ToolDefinition{
		...
		{Name: "probe_local_llm", Description: "Probe the local Ollama OpenAI-compatible endpoint, including whether a loaded model is GPU-accelerated (sizeVramBytes).", InputSchema: objectSchema(map[string]any{"includeChat": map[string]any{"type": "boolean"}}, nil)},
		{Name: "install_local_llm_model", Description: "Install Ollama (CUDA-pinned systemd unit) and pull an Ollama registry model.", InputSchema: objectSchema(map[string]any{"modelRef": map[string]any{"type": "string"}}, []string{"modelRef"})},
