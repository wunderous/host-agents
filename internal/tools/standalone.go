...
func StandaloneToolDefinitions() []ToolDefinition {
	defs := []ToolDefinition{
		...
		{Name: "run_host_command", Description: "Run a caller-declared generic host command for a serving or infrastructure assignment.", InputSchema: objectSchema(map[string]any{"command": map[string]any{"type": "string", "minLength": 1}}, []string{"command"})},
