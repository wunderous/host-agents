...
func StandaloneToolDefinitions() []ToolDefinition {
	defs := []ToolDefinition{
		...
		{Name: "inspect_host_service", Description: "Read systemd service state for a caller-declared host service without changing it.", InputSchema: objectSchema(map[string]any{"serviceName": map[string]any{"type": "string", "pattern": `^[A-Za-z0-9_.@:-]+$`}, "scope": map[string]any{"type": "string", "enum": []any{"user", "system"}}}, []string{"serviceName"}), OutputSchema: map[string]any{"type": "object", "required": []string{"serviceName", "scope", "status", "active", "enabled", "exitCode"}}},
		...
		{Name: "validate_host_plan", Description: "Validate a generic host-plan.v1 document against the current authorized capability catalog without changing host state.", InputSchema: objectSchema(map[string]any{"plan": map[string]any{}}, []string{"plan"})},
		{Name: "run_host_plan", Description: "Execute an explicit generic host-plan.v1 document with durable idempotency, readiness validation, recovery, and resume state.", InputSchema: objectSchema(map[string]any{"plan": map[string]any{}, "resume": map[string]any{"type": "boolean"}}, []string{"plan"})},
		{Name: "get_host_plan_run", Description: "Read the durable status, node results, and revision metadata for a host-plan.v1 run.", InputSchema: objectSchema(map[string]any{"runId": map[string]any{"type": "string", "minLength": 1}}, []string{"runId"})},
		{Name: "validate_runtime_recipe", Description: "Resolve and validate an external runtime-recipe.v1 source and its embedded host-plan.v1 without changing host state.", InputSchema: objectSchema(map[string]any{"source": map[string]any{"type": "string", "minLength": 1}, "revision": map[string]any{"type": "string"}, "sha256": map[string]any{"type": "string"}}, []string{"source"}), OutputSchema: map[string]any{"type": "object", "required": []string{"valid", "recipeId", "recipeVersion", "recipeHash", "rawSha256", "plan"}}},
		{Name: "run_runtime_recipe", Description: "Execute an externally sourced runtime-recipe.v1 through the existing durable host-plan runner; with activate=true, validate its neutral serving contract and commit it as the active runtime only after the run succeeds.", InputSchema: objectSchema(map[string]any{"source": map[string]any{"type": "string", "minLength": 1}, "revision": map[string]any{"type": "string"}, "sha256": map[string]any{"type": "string"}, "inputs": map[string]any{"type": "object"}, "resume": map[string]any{"type": "boolean"}, "runId": map[string]any{"type": "string", "minLength": 1}, "activate": map[string]any{"type": "boolean"}}, nil), OutputSchema: map[string]any{"type": "object", "required": []string{"runId", "status", "catalogRevision"}}},
		...
		{Name: "validate_tunnel_recipe", Description: "Resolve and validate an external tunnel-recipe.v1 source and its embedded host-plan.v1 without changing host state.", InputSchema: objectSchema(map[string]any{"source": map[string]any{"type": "string", "minLength": 1}, "revision": map[string]any{"type": "string"}, "sha256": map[string]any{"type": "string"}}, []string{"source"})},
		...
		{Name: "get_tunnel_run", Description: "Read durable tunnel-recipe.v1 provenance and host-plan state.", InputSchema: objectSchema(map[string]any{"runId": map[string]any{"type": "string", "minLength": 1}}, []string{"runId"}), OutputSchema: map[string]any{"type": "object", "required": []string{"runId", "status", "nodes", "recipe"}}},
		...
		{Name: "opute.provider.status", Description: "Read connected provider and active provider-generation state without changing host state.", InputSchema: objectSchema(map[string]any{"provider": map[string]any{"type": "string", "minLength": 1}}, []string{"provider"}), OutputSchema: map[string]any{"type": "object"}},
		{Name: "opute.provider.reload", Description: "Load a new trusted provider module generation and reconcile its selected recipe before activation.", InputSchema: objectSchema(map[string]any{"source": map[string]any{"type": "string", "minLength": 1}, "descriptor": map[string]any{"type": "object"}, "endpoint": map[string]any{"type": "string", "format": "uri"}, "token": map[string]any{"type": "string", "writeOnly": true}, "mode": map[string]any{"type": "string"}, "recipeSource": map[string]any{"type": "string"}, "revision": map[string]any{"type": "string"}, "sha256": map[string]any{"type": "string"}, "inputs": map[string]any{"type": "object"}, "activate": map[string]any{"type": "boolean"}}, nil), OutputSchema: map[string]any{"type": "object"}},
		{Name: "opute.provider.teardown", Description: "Ask a connected provider for a generic teardown plan, execute it durably after explicit confirmation, and retire the provider generation only after success.", InputSchema: objectSchema(map[string]any{"provider": map[string]any{"type": "string", "minLength": 1}, "generation": map[string]any{"type": "string", "minLength": 1}, "inputs": map[string]any{"type": "object"}, "confirm": map[string]any{"type": "boolean"}, "resume": map[string]any{"type": "boolean"}}, []string{"provider", "confirm"}), OutputSchema: map[string]any{"type": "object", "required": []string{"runId", "status", "catalogRevision"}}},
		{Name: "get_capability_catalog", Description: "Return the immutable authorized capability snapshot and revision used for typed proposals and host plans.", InputSchema: objectSchema(nil, nil)},
		{Name: "open_assistant_session", Description: "Negotiate the bounded assistant-session.v1 contract and bind the client to the current capability revision.", InputSchema: objectSchema(map[string]any{"sessionId": map[string]any{"type": "string", "minLength": 1}, "supportedContractVersions": map[string]any{"type": "array", "minItems": 1, "items": map[string]any{"type": "string"}}, "catalogRevision": map[string]any{"type": "string"}}, []string{"sessionId", "supportedContractVersions"})},
		...
		{Name: "put_k8s_secret", Description: "Create or replace a generic Kubernetes Secret without exposing its values in tool results.", InputSchema: objectSchema(map[string]any{"uri": map[string]any{"type": "string"}, "namespace": map[string]any{"type": "string"}, "name": map[string]any{"type": "string"}, "data": map[string]any{"type": "object", "additionalProperties": map[string]any{"type": "string"}}}, []string{"uri", "name", "data"})},
