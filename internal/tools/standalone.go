...
func StandaloneToolDefinitions() []ToolDefinition {
	defs := []ToolDefinition{
		...
		{Name: "install_local_llm_model", Description: "Install Ollama (CUDA-pinned systemd unit), pull a registry model (modelRef or modelPreset gemma|qwen), and optionally createAs a derived tag with numGpu/numCtx.", InputSchema: objectSchema(map[string]any{"modelRef": map[string]any{"type": "string"}, "modelPreset": map[string]any{"type": "string", "enum": []any{"gemma", "qwen"}}, "createAs": map[string]any{"type": "string"}, "numGpu": map[string]any{"type": "integer"}, "numCtx": map[string]any{"type": "integer"}, "template": map[string]any{"type": "string"}}, nil)},
