...
func StandaloneToolDefinitions() []ToolDefinition {
	defs := []ToolDefinition{
		...
		{Name: "run_host_plan", Description: "Execute an explicit generic host-plan.v1 document with durable idempotency, readiness validation, recovery, and resume state.", InputSchema: objectSchema(map[string]any{"plan": map[string]any{}, "resume": map[string]any{"type": "boolean"}}, []string{"plan"}), Meta: map[string]any{"resourceCost": map[string]any{"class": "normal", "cpuCores": 0.25, "memoryBytes": 268435456, "tasks": 1}}},
