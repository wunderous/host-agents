...
func StandaloneToolDefinitions() []ToolDefinition {
	defs := []ToolDefinition{
		...
		{Name: "configure_local_llm_model", Description: "Configure the alternate llama-server GGUF artifact manifest.", InputSchema: objectSchema(map[string]any{"runtime": map[string]any{"type": "string", "enum": []any{"llama-cpp"}}}, nil)},
