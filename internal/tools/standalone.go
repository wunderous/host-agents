...
func StandaloneToolDefinitions() []ToolDefinition {
	defs := []ToolDefinition{
		...
		{Name: "probe_local_llm", Description: "Probe the local Ollama endpoint. Optionally warm-load modelRef or modelPreset (phi|gemma|qwen) with numGpu/numCtx and return loadError/remediationHints plus GPU sizeVramBytes.", InputSchema: objectSchema(map[string]any{"includeChat": map[string]any{"type": "boolean"}, "modelRef": map[string]any{"type": "string"}, "modelPreset": map[string]any{"type": "string", "enum": []any{"phi", "gemma", "qwen"}}, "numGpu": map[string]any{"type": "integer"}, "numCtx": map[string]any{"type": "integer"}}, nil)},
		{Name: "install_local_llm_model", Description: "Install Ollama (CUDA-pinned systemd unit), pull a registry model (modelRef or modelPreset phi|gemma|qwen), and optionally createAs a derived tag with numGpu/numCtx and templatePreset (phi-functools for Phi tool calls).", InputSchema: objectSchema(map[string]any{"modelRef": map[string]any{"type": "string"}, "modelPreset": map[string]any{"type": "string", "enum": []any{"phi", "gemma", "qwen"}}, "createAs": map[string]any{"type": "string"}, "numGpu": map[string]any{"type": "integer"}, "numCtx": map[string]any{"type": "integer"}, "template": map[string]any{"type": "string"}, "templatePreset": map[string]any{"type": "string", "enum": []any{"phi-functools"}}}, nil)},
		{Name: "configure_local_llm_model", Description: "Create/replace a local Ollama tag FROM an already-pulled model with Modelfile parameters (numGpu/numCtx) and optional templatePreset (phi-functools). Does not re-download.", InputSchema: objectSchema(map[string]any{"modelRef": map[string]any{"type": "string"}, "modelPreset": map[string]any{"type": "string", "enum": []any{"phi", "gemma", "qwen"}}, "fromRef": map[string]any{"type": "string"}, "numGpu": map[string]any{"type": "integer"}, "numCtx": map[string]any{"type": "integer"}, "template": map[string]any{"type": "string"}, "templatePreset": map[string]any{"type": "string", "enum": []any{"phi-functools"}}}, nil)},
